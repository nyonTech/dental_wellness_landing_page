  ...
  return (
    <div className="bg-background-light font-display text-slate-900 antialiased overflow-x-hidden min-h-screen">
      <div className="relative flex min-h-screen w-full flex-col">
          ...
          <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="flex flex-col gap-6 text-center lg:text-left">
              ...
              <h1 className="text-slate-900 text-5xl md:text-6xl font-extrabold leading-[1.1] tracking-tight">
                Our Website is <br /> <span className="text-primary">Almost Ready</span>
              </h1>
              <p className="text-slate-600 text-lg md:text-xl leading-relaxed max-w-xl mx-auto lg:mx-0">
                We're refining our digital presence to match our world-class dental care. Subscribe to get notified and be the first to book your appointment.
              </p>
              {/* Trust Badges */}
            ...
            <div className="w-full max-w-md mx-auto lg:ml-auto">
              <div className="bg-white rounded-xl p-8 soft-shadow border border-slate-100 relative overflow-hidden">
                ...
                <p className="text-sm text-slate-500 mb-8">Secure your spot for our grand opening week.</p>
                <form ref={form} onSubmit={sendEmail} className="flex flex-col gap-5">
                  ...
                  <button disabled={status === "sending"} className="w-full bg-primary hover:bg-primary/90 text-slate-900 font-bold py-4 rounded-lg transition-all transform active:scale-[0.98] shadow-lg shadow-primary/30 flex items-center justify-center gap-2 mt-2 disabled:opacity-70 disabled:cursor-not-allowed" type="submit">
                    ...
                    {status === "sending" ? "Sending..." : "Notify Me & Request"}
                  </button>
                  {status === "success" && (
                  ...
                  <p className="text-[11px] text-center text-slate-400 mt-2 px-4">
                    By submitting, you agree to receive a one-time notification about our opening and appointment availability.
                  </p>
                </form>
              </div>
            </div>
        ...
        {/* Image Gallery / Secondary Visuals */}
        <section className="w-full px-4 lg:px-20 py-16 bg-white/50 backdrop-blur-sm border-t border-slate-100">
          <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="flex flex-col items-center text-center p-6 gap-4">
              <div className="size-16 rounded-full bg-primary/10 flex items-center justify-center">
                <Stethoscope className="w-8 h-8 text-primary" />
              </div>
              <h4 className="text-lg font-bold">General Dentistry</h4>
              <p className="text-sm text-slate-600">Comprehensive check-ups and cleanings for a lifetime of healthy smiles.</p>
            </div>
            <div className="flex flex-col items-center text-center p-6 gap-4 border-x border-slate-100">
              <div className="size-16 rounded-full bg-primary/10 flex items-center justify-center">
                <Sparkles className="w-8 h-8 text-primary" />
              </div>
              <h4 className="text-lg font-bold">Cosmetic Care</h4>
              <p className="text-sm text-slate-600">Whitening, veneers, and smile makeovers tailored to your unique beauty.</p>
            </div>
            <div className="flex flex-col items-center text-center p-6 gap-4">
              <div className="size-16 rounded-full bg-primary/10 flex items-center justify-center">
                <Activity className="w-8 h-8 text-primary" />
              </div>
              <h4 className="text-lg font-bold">Emergency Help</h4>
              <p className="text-sm text-slate-600">Urgent dental care when you need it most, with quick response times.</p>
            </div>
          </div>
        </section>
        {/* Footer */}
